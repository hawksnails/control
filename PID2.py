...
def animate(i):
    ...
    #最も近いマップ上の点を見つける
    nearest_index = np.argmin(np.sqrt((map.X - car.rear_x)**2 + (map.Y - car.rear_y)**2))
    nearest_x = map.X[nearest_index]
    nearest_y = map.Y[nearest_index]
    ...
    ax.plot(x_positions, y_positions, label="Vehicle Path")
    ax.plot(car.rear_x, car.rear_y, 'ro')  # Vehicle position
    ax.plot(nearest_x, nearest_y, 'go')  # Nearest point on the path
    ax.quiver(car.rear_x, car.rear_y, math.cos(car.yaw), math.sin(car.yaw), angles='xy', scale_units='xy', scale=1, color='b', headwidth=4)  # Vehicle orientation
    ax.legend()
    ax.set_xlim(-10, 110)
    ax.set_ylim(-50, 50)
    ax.set_title("Vehicle Position and Orientation")
...
fig, ax = plt.subplots()
ani = animation.FuncAnimation(fig, animate, frames=500, interval=200, repeat=False)
plt.show()